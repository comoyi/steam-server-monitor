...
package-android:
	ANDROID_HOME=~/Android/Sdk ANDROID_NDK_HOME=~/Android/Sdk/ndk/23.2.8568313 fyne package --release --target android --appID com.comoyi.steamservermonitor --name SteamServerMonitor --appVersion $(X_APP_VERSION)
	mkdir -p target/android
	mv SteamServerMonitor.aab target/android/SteamServerMonitor_$(X_APP_VERSION).aab
	bundletool build-apks --overwrite --mode=universal --bundle target/android/SteamServerMonitor_$(X_APP_VERSION).aab --output target/android/SteamServerMonitor_$(X_APP_VERSION).apks --ks ~/.jks/bundle.jks --ks-pass file:.jks-pass --ks-key-alias key1 --key-pass file:.jks-pass
	unzip -o -d target/android target/android/SteamServerMonitor_$(X_APP_VERSION).apks
	mv target/android/universal.apk target/android/SteamServerMonitor_$(X_APP_VERSION).apk
